...
def simulate_random_walk(graph, transition_matrix, steps=10000):
    ...
    for _ in range(steps):
        ...
        if neighbors:
            ...
            probabilities = np.array([transition_matrix[current_index, idx] for idx in neighbor_indices])
            probabilities /= probabilities.sum()  # Normalize probabilities
            current_node = np.random.choice(neighbors, p=probabilities)
    
    total_visits = sum(visit_counts.values())
    return {node: visit_counts[node] / total_visits for node in graph.nodes}
...
def compute_map_equation(graph, transition_matrix, partition):
    ...
    module_probs = defaultdict(float)
    ...
    for node, module in partition.items():
        module_probs[module] += visit_probs[node]
        for neighbor in graph.neighbors(node):
            if partition[neighbor] != module:
                node_idx = node_to_index[node]
                neighbor_idx = node_to_index[neighbor]
                exit_probs[module] += visit_probs[node] * transition_matrix[node_idx, neighbor_idx]

    H_modules = -sum(p * np.log2(p) for p in module_probs.values() if p > 0)
    H_exits = -sum(p * np.log2(p) for p in exit_probs.values() if p > 0)
    
    return H_exits + H_modules


def optimize_partition(graph, transition_matrix, iterations=100):
    ...
    for _ in range(iterations):
        for node in graph.nodes:
            ...
            for neighbor in graph.neighbors(node):
                ...
                new_score = compute_map_equation(graph, transition_matrix, partition)
